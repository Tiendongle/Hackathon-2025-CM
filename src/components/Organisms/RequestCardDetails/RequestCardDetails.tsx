...
const RequestCardDetails = ({ post }: RequestCardDetailsProps) => {
  ...
  const ratingDetailsData = [
    {
      detail: (
        <div className="flex justify-center items-center text-theme-orange">
  ...
  const ratingStarsDisplay = (starCount: number) => {
    ...
    for (let i = 1; i <= starCount; i++) {
      starArr.push(
        <Icon iconName="star-md" className="text-theme-orange w-3 h-3 " />
      );
    }
  ...
  return (
    <div className="w-full">
      ...
      <div className="w-11/12 m-auto flex gap-6 flex-col">
        ...
        <div className="flex flex-col gap-2">
          {post.bookings.map((booking, index) => (
            <Card variant="noBorder" key={index}>
              ...
              <button className="rounded-2xl bg-theme-orange text-white px-6">
                Inquire
              </button>
            </Card>
          ))}
        ...
        <div className="flex flex-col gap-2">
          <button className="rounded-2xl bg-theme-orange text-white p-4">
            Read all reviews
          </button>
